    ...
    try:
        ...
        if not settings:
            await callback.message.answer(
                "❌ Настройки WinkPay не найдены",
                reply_markup=get_yokassa_keyboard()
            )
            return

        message_text = (
            "<b>Текущие настройки WinkPay</b>\n\n"
        ...
        logger.error(f"Ошибка при отображении настроек WinkPay: {e}")
        await callback.message.answer(
            "Произошла ошибка при получении настроек WinkPay",
...
async def start_add_yokassa(callback: CallbackQuery, state: FSMContext):
    """Начало процесса добавления настроек WinkPay"""
    try:
        await callback.message.delete()
        ...
        await callback.message.answer(
            "Введите наименование для настроек WinkPay:",
        ...
        logger.error(f"Ошибка при начале добавления настроек WinkPay: {e}")
        await callback.message.answer(
...
async def process_api_key(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.info(f"Добавлены новые настройки WinkPay: {data['name']}")
        
    except Exception as e:
        logger.error(f"Ошибка при сохранении настроек WinkPay: {e}")
        await message.answer(
...
async def start_delete_yokassa(callback: CallbackQuery, state: FSMContext):
    """Начало процесса удаления настроек WinkPay"""
    try:
        await callback.message.delete()
        ...
        logger.error(f"Ошибка при начале удаления настроек WinkPay: {e}")
        await callback.message.answer(
            "Произошла ошибка. Попробуйте позже.",
            reply_markup=get_yokassa_keyboard()
        )
...
async def process_delete_shop_id(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"✅ Настройки WinkPay для {settings['name']} успешно удалены.",
            reply_markup=get_yokassa_keyboard()
        )
        logger.info(f"Удалены настройки WinkPay для {settings['name']} (SHOP ID: {shop_id})")
        
    except Exception as e:
        logger.error(f"Ошибка при удалении настроек WinkPay: {e}")
        await message.answer(
