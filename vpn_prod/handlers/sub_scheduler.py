            ...
            for sub in expiring_subs:
                try:
                    ...
                    message = (
                        ...
                        f"🌍 <b>Страна:</b> {server['name']}\n"
                        f"⏱ <b>Осталось:</b> {remaining_time}\n"
                        f"💰 <b>Цена продления:</b> {tariff['price']} ₽\n"
                        "</blockquote>\n"
                        "💡 Продлите подписку сейчас, чтобы не потерять доступ!"
                    )
                    
                    # Создаем inline кнопки с вариантами оплаты
                    keyboard = InlineKeyboardMarkup(inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text=f"💳 Продлить тот же тариф ({tariff['price']} ₽)",
                    ...
                    logger.info(f"Отправлено уведомление пользователю {sub['user_id']} о подписке {sub['id']}, тариф: {tariff['name']}, цена: {tariff['price']}")
